...
export const sendMailVerification = async (userDetails) => {
  try {
    ...
    const info = await transporter.sendMail({
      ...
      html: `Please click the following link to verify your email: <a href="http://localhost:3000/api/v1/users/register/verify-email/${verificationToken}">VERIFY</a> (This link will expire in ${process.env.VERIFICATION_TOKEN_EXPIRY} minutes)`, // html body
    });

    console.log("Message sent: %s", info.messageId);
    // Message sent: <d786aa62-4e0a-070a-47ed-0b0666549519@ethereal.email>
  } catch (error) {
    throw new ApiError(500, "Error while sending email to verify", error);
  }
};
